...
const validMethodName = (arrayWords: string[], callbackToUpperCaseFirstLetter: FirstLetter): string => {
...
const validMethodNumber = (arrayNumbers: number[], callbackMultiply: MultiplyNumber): string => {
...
const validMethodObject = (arrayObj: ObjNameAndAge[], callbackNameAndAge: NameAndAge): string => {
...
const validMethodReturnElement = (arrayElements: string[], callbackReverseElement: ReversElement) => {
...
const convertToObject: (number) => TypeConvertToObject = number => ({value: number, isOdd: Boolean(number % 2)});
