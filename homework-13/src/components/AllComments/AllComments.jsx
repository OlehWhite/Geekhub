    ...
    useEffect(() => {
        if (kidsComment.kids !== undefined) {
            const kidsComm = kidsComment.kids.map(_ =>
                fetch(`https://hacker-news.firebaseio.com/v0/item/${_}.json?print=pretty`)
                    .then(response => response.json())
                    .then(json => json))
            Promise.all(kidsComm)
                .then(response => setAllCommentsKids(response))
        }
    }, [kidsComment])
    ...
    <>
        <Box className="kids-comment">
            ...
            <Box className="all-comments-kids">
                ...
                {allCommentsKids.length > 0 ? allCommentsKids.map((item, index) => (
